...
struct Wall{
	...
	Wall(int pt1, int pt2, double len){

	}
};

void detectWalls(std::vector<Wall>& allWalls, Point pt1, Point pt2, Point middle, int width, int height) {
	...
	while (runbot) {
		...
		for(size_t i = 0; i < lines.size(); i++) {
			...
			if(pos.y > 70 && sqrt(pow(pos.x - width*.5, 2) + pow(pos.y - height*.5, 2)) > 120){
				detectWalls(allWalls, Point(l[0], l[1]), Point(l[2], l[3]), pos, width, height);
